...
This project is intended to demonstrate entry-level QA planning, manual functional testing, defect documentation, requirements traceability, and beginner-to-junior-level Playwright automation.
...
- Users can sort inventory by name and price.
- Users can open product detail pages from the inventory page.
...
| Inventory | Product display, product detail navigation, sorting, add-to-cart behavior, cart badge behavior |
| Cart | Product name, quantity, price verification, and remove-from-cart behavior |
...
| Automation | Beginner-to-junior Playwright smoke/regression tests for core flows |
...
- Observed defects are documented with steps, expected result, actual result, severity, priority, and impact.
...
- Manual coverage, automation coverage, and limitations are clearly separated.
...
| `automation-strategy.md` | Automation scope, selector strategy, and CI notes |
...
| Automation coverage is beginner-to-junior level | Not all manual scenarios are automated | Clearly separate manual coverage from automation coverage |
...
2. Create manual test cases for positive, negative, validation, sorting, cart, checkout, product-detail, and defect-focused scenarios.
...
6. Automate repeatable smoke/regression scenarios using Playwright.
...
This test plan supports an entry-level QA portfolio by showing that the tester can plan QA work, define testing scope, execute manual tests, report defects, understand traceability, and build beginner-to-junior-level Playwright automation for repeatable functional flows.
